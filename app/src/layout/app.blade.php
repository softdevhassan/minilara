@php
    ...
    $svgIcon =
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect width="100" height="100" rx="20" fill="' .
        $userColors['bp'] .
        '" /><text x="50%" y="50%" dominant-baseline="central" text-anchor="middle" fill="' .
        $userColors['accent'] .
        '" font-family="Rubik, sans-serif" font-weight="900" font-size="50">' .
        $iconText .
        '</text></svg>';
    $useDynamic = ($settings['APP_USE_DYNAMIC_FAVICON'] ?? '0') === '1';
    $faviconData = $useDynamic
        ? 'data:image/svg+xml;base64,' . base64_encode(trim($svgIcon))
        : asset($settings['APP_FAVICON'] ?? '');
    $siteName = $settings['APP_NAME'] ?? 'Mini Lara';
    $separator = ' | ';
    ...
    foreach ($menu as $item) {
        if (isset($item['children'])) {
            foreach ($item['children'] as $child) {
                $flatMenu[] = ['title' => $child['title'], 'url' => url($child['url']), 'parent' => $item['title']];
            }
        } else {
            $flatMenu[] = ['title' => $item['title'], 'url' => url($item['url']), 'parent' => null];
        }
    }
@endphp
<!DOCTYPE html>
...
<head>
    ...
    <meta name="generator" content="Mini Lara v1.3.4 (Industrial Edition)" />
...
@php
    $isPrint = strpos($cleanRoute, '/print/') === 0;
    $bodyClass = $isPrint ? 'bg-white text-slate-900' : 'bg-bp text-tp';
@endphp
...
<body
    ...
    <!-- SEARCH MODAL -->
    <div x-show="searchOpen" x-cloak class="fixed inset-0 z-[100] overflow-y-auto p-4 sm:p-6 md:p-20" role="dialog"
        aria-modal="true">
        ...
        <div x-show="searchOpen" x-transition:enter="ease-out duration-300"
            ...
            <div class="relative">
                ...
                <i class="ri-loader-4-line absolute left-4 top-3.5 text-accent text-xl animate-spin"
                    x-show="isSearching"></i>
                        ...
                        <div :class="searchIndex === index ?
                            ...
                            <a :href="item.url" class="flex-auto flex items-center min-w-0">
                                ...
                                <div class="ml-4 flex-auto min-w-0">
                                    ...
                                    <div class="flex items-center gap-2 mt-0.5">
                                        <p class="text-[9px] font-bold text-accent uppercase"
                                            x-text="item.parent || 'System'"></p>
                                        <template x-if="item.subtitle">
                                            <div class="flex items-center gap-2">
                                                <span class="w-1 h-1 bg-ts/20 rounded-full"></span>
                                                <p class="text-[10px] font-bold text-tp truncate"
                            ...
                            <div class="flex items-center gap-1.5 ml-4 no-print">
                                ...
                                <template x-if="item.print_url">
                                    <a :href="item.print_url" target="_blank"
                                        class="w-8 h-8 flex items-center justify-center rounded-lg border border-ts/10 text-ts hover:border-accent hover:text-accent hover:bg-accent/5 transition-colors"
                                        title="Print">
            ...
            <div x-show="searchQuery !== '' && filteredItems.length === 0 && !isSearching"
    ...
    <div class="{{ $isPrint ? '' : 'h-full flex overflow-hidden w-full' }}">
        @if ($showLayout && !$isPrint)
            ...
            <div x-show="mobileOpen" @click="mobileOpen = false"
                ...
                class="fixed inset-0 z-40 bg-black/60 md:hidden" x-cloak></div>
            <aside :class="mobileOpen ? 'translate-x-0' : '-translate-x-full'"
                ...
                <!-- Logo -->
                <div
                    class="h-16 flex items-center justify-between px-3 border-b border-white/5 relative bg-[var(--sb)] flex-shrink-0">
                    <a href="{{ url('/home') }}" class="flex items-center flex-1 justify-center">
                        ...
                        @else
                            <span
                                class="text-base font-bold text-white">{{ $settings['APP_NAME'] ?? 'Mini Lara' }}</span>
                        @endif
                    </a>
                        ...
                        @if ($showItem)
                            ...
                            @else
                                <a href="{{ url($item['url']) }}"
                                    ...
                                    <i
                                        class="{{ $item['icon'] }} text-base transition-colors {{ $isActive($item['url']) ? 'text-[var(--st)]' : 'text-[var(--sl)] group-hover:text-[var(--sh)]' }}"></i>
                                    <span class="font-medium text-[13px]">{{ $item['title'] }}</span>
                                </a>
                            @endif
                        @endif
                ...
                <div class="flex-shrink-0 px-3 py-3 border-t border-white/5 bg-[var(--sb)]">
                    <div class="flex flex-col items-center justify-center gap-1">
                        <span class="text-white/70 text-[9px] font-medium uppercase">&copy; {{ date('Y') }}
                            {{ get_setting('APP_NAME', 'Mini Lara') }}</span>

                        @php
                            $devName = $_ENV['DEVELOPER_NAME'] ?? '';
                            $devUrl = $_ENV['DEVELOPER_URL'] ?? '#';
                            $compName = $_ENV['DEVELOPED_BY_COMPANY'] ?? '';
                            $compUrl = $_ENV['DEVELOPED_BY_COMPANY_URL'] ?? '#';
                        @endphp
                        ...
                        <span class="text-[8px] text-white/50 uppercase font-medium">
                            ...
                            @if ($devName && $compName)
                                <a href="{{ $devUrl }}" target="_blank"
                                    class="text-white/70 hover:underline">{{ $devName }}</a>
                                at
                                <a href="{{ $compUrl }}" target="_blank"
                                    class="text-white/70 hover:underline">{{ $compName }}</a>
                            @elseif ($devName)
                                <a href="{{ $devUrl }}" target="_blank"
                                    class="text-white/70 hover:underline">{{ $devName }}</a>
                            @elseif ($compName)
        ...
        <div
                ...
                <header
                    ...
                    <div class="px-3 sm:px-5 h-full flex items-center gap-3">
                        ...
                        <div class="w-full max-w-xs relative group">
                            <i
                                class="ri-search-2-line absolute left-3 top-1/2 -translate-y-1/2 text-ts/50 group-hover:text-accent transition-colors text-sm"></i>
                            <input type="text" @click="toggleSearch()" readonly
                            ...
                            <div class="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                                <span
                                    class="text-[8px] font-bold text-ts/40 bg-bp px-1 py-0.5 rounded border border-ts/10">Ctrl</span>
                                <span
                                    class="text-[8px] font-bold text-ts/40 bg-bp px-1 py-0.5 rounded border border-ts/10">K</span>
                            </div>
                        </div>
                            ...
                            <div x-data="{ profileOpen: false }" class="relative">
                                ...
                                <div x-show="profileOpen" @click.away="profileOpen = false" x-cloak
                                    ...
                                    <div class="px-4 py-3 border-b border-ts/5 mb-1">
                                        <div class="text-[14px] font-bold text-tp truncate">
                                            {{ $user['name'] ?? 'Admin' }}</div>
                                        <div class="text-[12px] font-bold text-ts tracking-tight mt-0.5">
                                            <span>@</span>{{ $user['username'] ?? 'username' }}</div>
                                    </div>
            ...
            <main
                ...
                @if ($isPrint)
                    <!-- CENTRALIZED PRINT FRAME -->
                    <div class="no-print print-bar">
                        <h1 class="text-sm font-bold uppercase tracking-tight">{{ $title ?? 'Print Document' }}</h1>
                        <div class="flex items-center gap-3">
                            <a href="javascript:history.back()" class="print-btn-white">Back</a>
                            <button onclick="window.print()" class="print-btn-black">Print</button>
                        </div>
                    </div>

                    <div class="a4-container {{ $printClass ?? '' }}">
                        <!-- Core Content (Header, Body, Footer handled by individual files) -->
                        @yield ('content')
                    </div>
                @else
                    @yield ('content')
